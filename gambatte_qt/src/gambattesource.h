...
public:
	...
	int externalRead(unsigned short addr) {return gb_.externalRead(addr); }
	void externalWrite(unsigned short addr, unsigned char val) {return gb_.externalWrite(addr, val); }
...
private:
	...
	void emitRamShare() { if(!isResetting_) { emit ramShare();} }
